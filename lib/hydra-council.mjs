 ...
 * Hydra Council Mode
 *
 * Agent-aware multi-round deliberation. Requested participants are resolved to
 * installed backends, so the default flow also works with Claude + Codex only.
...
import { getAgent, getMode, setMode } from './hydra-agents.mjs';
import { resolveProject, loadHydraConfig } from './hydra-config.mjs';
import { resolveAvailableAgent } from './hydra-capabilities.mjs';
...
/**
 * Preferred council flow. Gemini remains supported, but is not required: each
 * step is resolved through the configured physical-agent fallback chain.
 */
const COUNCIL_FLOW = [
...
/** Resolve the preferred flow to the physical backends available now. */
export function resolveCouncilFlow(flow = COUNCIL_FLOW, hydraConfig = loadHydraConfig(), opts = {}) {
  return flow
    .map((step) => {
      const agent = resolveAvailableAgent(step.agent, hydraConfig, opts);
      return agent ? { ...step, requestedAgent: step.agent, agent } : null;
    })
    .filter(Boolean);
}
...
async function callAgentAsync(agent, prompt, timeoutMs) {
  ...
  return {
    ...
    requestedAgent: result.requestedAgent || agent,
    fallbackAgent: result.fallbackAgent || null,
  };
}
...
/** Resolve council participants to distinct execution backends available now. */
export function resolveCouncilAgents(agentsFilter, hydraConfig = loadHydraConfig(), opts = {}) {
  const requested = resolveActiveAgents(agentsFilter);
  const resolved = requested
    .map((agent) => resolveAvailableAgent(agent, hydraConfig, opts))
    .filter(Boolean);
...
async function runAdversarialCouncil(prompt, report, { preview, timeoutMs, specContent, promptHash, agentsFilter, rounds }) {
  const activeAgents = resolveCouncilAgents(agentsFilter);
...
async function main() {
  ...
  const preferredFlow = agentsFilter
    ? COUNCIL_FLOW.filter((step) => agentsFilter.includes(step.agent))
    : COUNCIL_FLOW;
  const activeFlow = resolveCouncilFlow(preferredFlow);
  ...
  const report = {
    ...
    publish,
    rounds,
    councilFlow: activeFlow.map((step) => `${step.agent}:${step.phase}`),
    url,
  ...
  report.tasks = report.tasks.map((task) => {
    if (task.owner === 'human' || task.owner === 'unassigned') return task;
    return { ...task, owner: resolveAvailableAgent(task.owner, runtimeConfig) || 'unassigned' };
  });
  ...
  if (publish) {
    try {
      ...
      const decision = await request('POST', url, '/decision', {
        ...
        impact: `Rounds=${rounds}; Tasks=${createdTasks.length}; Flow=${report.councilFlow.join(' -> ')}; next=${report.recommendedNextAction || report.recommendedMode}`,
      });

      const handoffs = [];
      const publishAgents = resolveCouncilAgents(agentsFilter, runtimeConfig);
