  ...
  codex: {
    ...
    invoke: {
      ...
      headless: (prompt, opts = {}) => {
        ...
        } else if (opts.permissionMode === 'read-only' || opts.permissionMode === 'plan') {
...
export function getVerifier(producerAgent) {
  ...
  const defaults = { gemini: 'claude', codex: 'claude', claude: 'codex' };
 ...
 */
export function initAgentRegistry(configOverride = null) {
  ...
  try {
    const cfg = configOverride || loadHydraConfig();
...
// Generic/legacy display capabilities; structured CLI discovery may provide a
// more precise per-model effort list to interactive selectors.
...
export function getActiveModel(agentName) {
  ...
  const normalize = (modelId) => {
    ...
    return resolveModelId(agentName, modelId) || modelId;
  };
...
export function getModelFlags(agentName) {
  ...
  if (effort) {
    if (agentName === 'codex') {
      flags.push('--config', `model_reasoning_effort=${JSON.stringify(effort)}`);
...
export function getModelSummary() {
  ...
  for (const agent of orderedAgents) {
    ...
    summary[agent] = {
      active: activeModel || agentModels.default || (agent === 'local' ? 'automatic' : 'cli-default'),
