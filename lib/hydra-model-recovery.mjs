...
export async function verifyAgentQuota(agent, opts = {}) {
  ...
  try {
    if (agent === 'codex') {
      ...
      //   1. ChatGPT Codex quota (chatgpt.com/codex) — for Codex CLI models
      //      Managed via the ChatGPT web auth, NOT the OpenAI REST API.
      //      Cannot be verified with OPENAI_API_KEY.
      //   2. OpenAI API quota (api.openai.com) — for API-based models
...
export async function recoverFromModelError(agent, failedModel, opts = {}) {
  ...
  let candidates = getFallbackCandidates(agent, failedModel);
  try {
    const { fetchModels } = await import('./hydra-models.mjs');
    const discovered = await fetchModels(agent);
    const failed = String(failedModel || '').toLowerCase();
    const dynamic = (discovered.models || [])
      .filter((id) => id && id.toLowerCase() !== failed && !isCircuitOpen(id))
      .map((id) => ({ id, label: `discovered: ${id}`, source: 'discovery', qualityScore: 0 }));
    const seen = new Set(dynamic.map((candidate) => candidate.id.toLowerCase()));
    candidates = [
      ...dynamic,
      ...candidates.filter((candidate) => !seen.has(candidate.id.toLowerCase()) && !isCircuitOpen(candidate.id)),
    ];
  } catch {
    candidates = candidates.filter((candidate) => !isCircuitOpen(candidate.id));
  }
