...
import { isAgentAvailable, resolveAvailableAgentNames } from './hydra-capabilities.mjs';
import {
  parseArgs,
  getPrompt,
  parseList,
...
/** Resolve logical/requested participants before any observable dispatch side effect. */
export function resolveDispatchAgents(agentNames, opts = {}) {
  const runtimeConfig = opts.runtimeConfig || loadHydraConfig();
  return resolveAvailableAgentNames(agentNames, runtimeConfig, {
    ...(opts.capabilityOpts || {}),
    resolvePhysicalAgent,
  });
}

export function startAgentWorker(agent, baseUrl, opts = {}) {
  const { rl } = opts;
  const [name] = resolveDispatchAgents([agent], opts);
  if (!name) {
    console.log(`  ${WARNING('!')} ${String(agent || '').toLowerCase()} worker skipped ${DIM('(agent unavailable)')}`);
    return null;
  }
...
export function startAgentWorkers(agentNames, baseUrl, opts = {}) {
  const resolvedAgents = resolveDispatchAgents(agentNames, opts);
  const startWorker = opts.startWorkerFn || startAgentWorker;
  for (const agent of resolvedAgents) {
    startWorker(agent, baseUrl, opts);
  }
...
async function publishMiniRoundDelegation({ baseUrl, from, agents, promptText, report }) {
  agents = resolveDispatchAgents(agents);
  if (agents.length === 0) throw new Error('No installed agents are available for handoff.');
  ...
  for (const task of tasksToCreate) {
    const owner = ['human', 'unassigned'].includes(task.owner)
      ? task.owner
      : (resolveDispatchAgents([task.owner])[0] || agents[0]);
...
export async function dispatchPrompt({
  ...
  const resolvedAgents = resolveDispatchAgents(agents, { runtimeConfig, capabilityOpts });
  if (resolvedAgents.length === 0) throw new Error('No installed agents are available for handoff.');
  const records = [];
  for (const agent of resolvedAgents) {
    ...
    const result = await requestFn('POST', baseUrl, '/handoff', payload);
    records.push({
      agent,
...
async function publishFastPathDelegation({ baseUrl, from, promptText, classification, agents = null }) {
  agents = resolveDispatchAgents(agents || AGENT_NAMES);
  if (agents.length === 0) throw new Error('No installed agents are available for handoff.');
...
async function runAutoPrompt({ baseUrl, from, agents, promptText, miniRounds, councilRounds, preview, onProgress = null }) {
  agents = resolveDispatchAgents(agents);
  if (agents.length === 0) throw new Error('No installed agents are available for dispatch.');
    ...
    try {
      ...
      if (line === ':model' || line.startsWith(':model ')) {
        ...
        if (modelArgs) {
          ...
          console.log(DIM(`  Override:  :model codex=gpt-5.6-sol`));
      ...
      if (line === ':agents' || line.startsWith(':agents ')) {
        ...
        if (!agentSubCmd) {
          ...
          for (const a of physical) {
            ...
            const available = isAgentAvailable(a.name, loadHydraConfig());
            const availability = available ? SUCCESS('available') : WARNING('not installed');
            console.log(`    ${colorAgent(a.name)} ${DIM(a.label)}  ${DIM('model:')} ${pc.white(model)}  [${availability}]`);
          }
          ...
          for (const a of list) {
            ...
            const status = a.enabled ? SUCCESS('on') : ERROR('off');
            const physical = resolvePhysicalAgent(a.name);
            const available = physical && isAgentAvailable(physical.name, loadHydraConfig());
            const availability = available ? SUCCESS('available') : WARNING('unavailable');
            console.log(`    ${colorAgent(a.name)} ${typeTag} ${DIM(a.displayName)}  [${status}] [${availability}]`);
          }
...
async function main() {
  ...
  const requestedAgents = parseList(options.agents || 'gemini,codex,claude');
  ...
  const agents = resolveDispatchAgents(requestedAgents);
  if (agents.length === 0) {
    throw new Error('No installed agents are available. Install Claude Code, Codex CLI, or Gemini CLI.');
  }
...
if (path.basename(process.argv[1] || '') === 'hydra-operator.mjs') {
  main().catch((error) => {
