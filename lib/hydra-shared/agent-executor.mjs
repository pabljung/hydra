...
import { getActiveModel, getReasoningEffort, getAgent, resolvePhysicalAgent } from '../hydra-agents.mjs';
...
import { hasGeminiOAuthCredentials, resolveAvailableAgent } from '../hydra-capabilities.mjs';
import {
 ...
 */
export function isSafeModelId(modelId) {
  return typeof modelId === 'string' && Boolean(modelId.trim()) && modelId.length <= 256 && !/[\0\r\n]/.test(modelId);
}

/** Build the exact headless CLI invocation without starting a process. */
export function buildCliInvocation(agent, prompt, opts = {}) {
  const effectiveModel = opts.modelOverride || getActiveModel(agent) || null;
  const effortOverride = opts.reasoningEffort;

  if (agent === 'codex') {
    const args = ['exec', '-'];
    if (opts.permissionMode === 'read-only' || opts.permissionMode === 'plan') {
      args.push('--sandbox', 'read-only');
    } else if (opts.permissionMode === 'full-auto') {
      args.push('--dangerously-bypass-approvals-and-sandbox');
    } else {
      // Codex CLI 0.153+ replaced the old --full-auto shorthand.
      // --approve-for-me keeps automatic approval inside workspace-write.
      args.push('--approve-for-me');
    }
    if (opts.cwd) args.push('-C', opts.cwd);
    if (effectiveModel) args.push('--model', effectiveModel);
    const effort = effortOverride || getReasoningEffort('codex');
    if (effort) {
      args.push('--config', `model_reasoning_effort=${JSON.stringify(effort)}`);
    }
    args.push('--json');
    return { cmd: 'codex', args, useStdin: true, effectiveModel };
  }

  if (agent === 'gemini') {
    const args = ['-p', prompt, '--approval-mode', opts.permissionMode || 'auto-edit', '-o', 'json'];
    if (effectiveModel) args.push('--model', effectiveModel);
    return { cmd: 'gemini', args, useStdin: false, effectiveModel };
  }

  const permission = resolveClaudePerm(opts.permissionMode || 'auto-edit');
  const useStdin = opts.useStdin !== false;
  const args = useStdin
    ? ['--output-format', 'json', '--permission-mode', permission]
    : ['-p', prompt, '--output-format', 'json', '--permission-mode', permission];
  if (effectiveModel) args.push('--model', effectiveModel);
  const effort = effortOverride || getReasoningEffort('claude');
  if (effort) {
    args.push('--effort', effort);
  }
  return { cmd: 'claude', args, useStdin, effectiveModel };
}

/** Spawn an already-built CLI invocation with a direct argv and no shell. */
export function spawnCliInvocation(invocation, options = {}, spawnImpl = spawn) {
  return spawnImpl(invocation.cmd, invocation.args, {
    ...options,
    shell: false,
  });
}
...
export async function executeAgent(agent, prompt, opts = {}) {
  const requestedAgent = String(agent || '').toLowerCase();
  const requestedDef = getAgent(requestedAgent);
  const physicalAgent = resolvePhysicalAgent(requestedAgent)?.name || requestedAgent;
  const physicalDef = getAgent(physicalAgent);
  const runtimeConfig = loadHydraConfig();
  const availableAgent = physicalDef?.customType
    ? physicalAgent
    : resolveAvailableAgent(physicalAgent, runtimeConfig, opts);
  ...
  if (!availableAgent) {
    return {
      ...
      output: '',
      stdout: '',
      stderr: `No installed execution backend is available for ${requestedAgent}`,
      error: `Agent unavailable: ${requestedAgent}`,
      errorCategory: 'agent-unavailable',
      exitCode: null,
      signal: null,
      durationMs: 0,
      timedOut: false,
  ...
  if (availableAgent !== requestedAgent) {
    const fallbackOpts = { ...opts, _agentFallback: true };
    if (availableAgent !== physicalAgent) delete fallbackOpts.modelOverride;
    const resolvedPrompt = requestedDef?.type === 'virtual' && requestedDef.rolePrompt && !opts._resolvedVirtual
      ? `${requestedDef.rolePrompt}\n\n---\n\nTask:\n${prompt}`
      : prompt;
    fallbackOpts._resolvedVirtual = true;
    const result = await executeAgent(availableAgent, resolvedPrompt, fallbackOpts);
    const usedAgentFallback = availableAgent !== physicalAgent;
    return {
      ...result,
      requestedAgent,
      virtualAgent: requestedDef?.type === 'virtual' ? requestedAgent : undefined,
      fallbackAgent: usedAgentFallback ? availableAgent : (result.fallbackAgent || null),
      agentFallback: usedAgentFallback || result.agentFallback || false,
    };
  }
  ...
  // Prefer the existing direct OAuth transport when credentials are present.
  if (agent === 'gemini' && hasGeminiOAuthCredentials(opts)) {
    try {
  ...
  if (modelOverride && !isSafeModelId(modelOverride)) {
    _hubCleanup();
    return {
      ok: false,
      output: '',
  ...
  const effectiveModel = modelOverride || getActiveModel(agent) || null;
  const metricsModel = effectiveModel || 'cli-default';

  // OTel tracing
  const spanPromise = startAgentSpan(agent, metricsModel, {
    phase: phaseLabel,
    taskType: opts.taskType,
  });

  // Metrics recording
  const metricsHandle = recordCallStart(agent, metricsModel);

  return new Promise((resolve) => {
    const invocation = buildCliInvocation(agent, prompt, {
      cwd,
      modelOverride,
      reasoningEffort: effortOverride,
      permissionMode,
    ...
    const useStdinForPrompt = invocation.useStdin;
