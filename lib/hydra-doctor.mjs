...
async function createFollowUp(failure, diagnosis, type) {
  ...
  const preferredAgent = diagnosis.action === 'fix' ? 'codex' : 'claude';
...
function selectFixAgent(entry) {
  ...
  // Keep diagnosis independent from the failed backend on a Claude + Codex install.
  if (failedAgent === 'claude') return 'codex';
  return 'claude';
}
