...
import { resolveModelId } from './hydra-agents.mjs';
import { getAvailableAgentNames } from './hydra-capabilities.mjs';
...
const installedAuditAgents = getAvailableAgentNames(cfg).filter((agent) => ['claude', 'gemini', 'codex'].includes(agent));
const AGENTS = parseCsv(args.agents || (installedAuditAgents.join(',') || 'claude,codex'));
...
async function dispatchToAgent(agent, prompt, projectPath, economy, timeoutMs) {
  const startedAt = Date.now();
  const cheapModel = economy ? cfg.models?.[agent]?.cheap : null;
  const modelOverride = cheapModel ? resolveModelId(agent, cheapModel) : undefined;
  ...
  if (VERBOSE) {
    console.log(`  [${agent}] ${result.ok ? 'ok' : result.error || 'failed'} (${elapsedSec}s, ${(result.output || '').length} chars)`);
  }
  return {
    agent,
