...
import { initAgentRegistry, classifyTask, bestAgentFor, getActiveModel, resolveModelId } from './hydra-agents.mjs';
...
async function main() {
  ...
  if (!noDiscovery) {
    ...
    const discoveryAgent = String(options['discovery-agent'] || discoveryCfg.agent || 'claude');
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    const cheapModel = cfg.models?.[agent]?.cheap;
    const modelOverride = useEconomy
      ? (agent === budgetCfg.handoffAgent && budgetCfg.handoffModel
        ? resolveModelId(agent, budgetCfg.handoffModel)
        : resolveModelId(agent, cheapModel))
      : undefined;
