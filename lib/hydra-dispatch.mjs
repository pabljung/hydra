 ...
 * 2) Best available reviewer (Gemini preferred when installed)
...
async function callAgent(agent, prompt, timeoutMs) {
  usageGuard(agent);
  return executeAgentWithRecovery(agent, prompt, { cwd: config.projectRoot, timeoutMs });
}
...
function buildClaudeCoordinatorPrompt(userPrompt, daemonSummary) {
  ...
  return [
    ...
    '',
    'Create a high-quality delegation plan for the available reviewer, Codex, and Claude.',
    ...
    '',
    'Requirements:',
    '1) Generate specific prompts for the reviewer and Codex.',
...
function buildReviewerPrompt(reviewerAgent, userPrompt, claudePlan, daemonSummary) {
  const agentConfig = getAgent(reviewerAgent);

  return [
    `${isPersonaEnabled() ? getAgentFraming(reviewerAgent) : `You are ${agentConfig.label}`} Independent review mode for ${config.projectName}.`,
    '',
    agentConfig.rolePrompt,
    '',
    getProjectContext(reviewerAgent, {}, config),
    '',
...
function buildCodexPrompt(userPrompt, claudePlan, reviewerOutput, daemonSummary) {
...
async function main() {
  ...
  const reviewerAgent = resolveAvailableAgent('gemini', hydraConfig) || 'claude';
  ...
  if (isPreview) {
    ...
    const reviewerConfig = getAgent(reviewerAgent);
    ...
    report.gemini = {
      ...
      fallbackAgent: reviewerAgent === 'gemini' ? null : reviewerAgent,
      command: reviewerConfig.invoke.nonInteractive('<reviewer-prompt>'),
    };
    report.codex = {
      ok: true,
      preview: true,
    ...
    const geminiPrompt = buildReviewerPrompt(reviewerAgent, prompt, claudeParsed || claudeResult.stdout, report.daemonSummary);
    const spinGemini = createSpinner(`${colorAgent(reviewerAgent)} ${DIM('critiquing...')}`, { style: 'solar' });
    spinGemini.start();
    const geminiResult = await callAgent(reviewerAgent, geminiPrompt, timeoutMs);
    const geminiParsed = parseJsonLoose(geminiResult.stdout);
    report.gemini = {
      ...geminiResult,
      parsed: geminiParsed,
    };
    geminiResult.ok ? spinGemini.succeed(`${colorAgent(reviewerAgent)} critique complete`) : spinGemini.fail(`${colorAgent(reviewerAgent)} critique failed`);
  ...
  console.log(label(`Reviewer (${reviewerAgent})`, report.outputSummary.geminiOk ? SUCCESS('ok') : ERROR('failed')));
  ...
  console.log(label('Reviewer snippet', DIM(report.outputSummary.geminiSnippet)));
