 ...
 * @param {string} modelSpec - Model ID or configured alias
 */
export function switchConciergeModel(modelSpec) {
  const cfg = loadHydraConfig();
  const spec = modelSpec.toLowerCase();
  const providerByAgent = { codex: 'openai', claude: 'anthropic', gemini: 'google' };
  let configured = cfg.concierge?.aliases?.[spec];
  if (!configured) {
    for (const [agent, aliases] of Object.entries(cfg.aliases || {})) {
      if (aliases?.[spec]) {
        configured = { provider: providerByAgent[agent], model: aliases[spec] };
        break;
      }
    }
  }
  const resolved = typeof configured === 'string' ? configured : configured?.model || modelSpec;

  // Detect provider from model name
  let provider = 'openai';
  if (configured?.provider) provider = configured.provider;
  else if (resolved.startsWith('claude-') || ['opus', 'sonnet', 'fable'].includes(spec)) provider = 'anthropic';
