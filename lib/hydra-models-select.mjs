...
export async function pickModel(agentName) {
  ...
  const { models, source, capabilities = {} } = await fetchModels(agentName);
  for (const [modelId, value] of Object.entries(capabilities)) {
    discoveredCapabilities.set(`${agentName}:${modelId}`, value);
  }
  ...
  // Build item list: presets, explicit active model, aliases, then discovery.
  const seen = new Set();
  const items = [];
  ...
  if (currentModel && !seen.has(currentModel)) {
    ...
    items.push({ id: currentModel, preset: null, active: true });
  }
  for (const id of Object.values(aliases)) {
    if (id && !seen.has(id)) {
      seen.add(id);
...
export async function pickEffort(agentName, modelId) {
  ...
  const discovered = discoveredCapabilities.get(`${agentName}:${effectiveModel}`);
  const options = discovered?.reasoningEfforts?.length
    ? [
        { id: null, label: `default${discovered.defaultReasoningEffort ? ` (${discovered.defaultReasoningEffort})` : ''}`, hint: 'Use the CLI model default' },
        ...discovered.reasoningEfforts.map((entry) => ({ id: entry.id, label: entry.id, hint: entry.description || null })),
      ]
    : getEffortOptionsForModel(effectiveModel);
