...
import { getRoleConfig, loadHydraConfig, resolveProject } from './hydra-config.mjs';
...
async function phaseExecute(tasks, projectRoot, cfg, startedAt) {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    let modelOverride;
    if (useHandoff) {
      const handoffRole = getRoleConfig('nightlyHandoff');
      agent = budgetCfg.handoffAgent || handoffRole.agent;
      modelOverride = budgetCfg.handoffModel ?? handoffRole.model ?? undefined;
    } else {
