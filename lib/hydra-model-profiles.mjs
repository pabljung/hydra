/**
 * Hydra Model Profiles — Optional benchmark and compatibility metadata.
 *
 * Encodes the historical MODEL_PROFILES.md snapshot for pricing and UI
 * annotations. MODEL_PROFILES is not an allowlist. Runtime defaults and model
 * discovery are deliberately independent so unknown/current CLI IDs pass through.
 *
...
export const ROLE_DEFAULTS = {
  architect: {
    ...
    models: ['opus', 'sonnet'],
    note: 'Claude Code aliases track the models available to the installed CLI.',
  },
  analyst: {
    agent: 'claude',
    model: null,
    reasoningEffort: null,
    models: ['opus', 'sonnet'],
    note: 'Defaults to Claude so analysis works when Gemini is not installed.',
  },
  implementer: {
    agent: 'codex',
    model: null,
    ...
    note: 'Uses models reported by the installed Codex CLI structured catalog.',
  },
  concierge: {
    agent: 'codex',
    model: 'gpt-5.6-luna',
    ...
    note: 'Fast current models for direct API concierge providers.',
  },
  investigator: {
    agent: 'codex',
    model: 'gpt-6-astra',
    ...
    note: 'High-effort Codex CLI diagnosis with model recovery.',
  },
  nightlyHandoff: {
    agent: 'codex',
    model: 'gpt-5.6-luna',
    ...
    note: 'Fast Codex models confirmed by the installed CLI catalog.',
  },
};
...
export const AGENT_PRESETS = {
  claude: { default: 'opus', fast: 'sonnet', cheap: 'sonnet' },
  codex:  { default: 'gpt-6-astra', fast: 'gpt-5.6-luna', cheap: 'gpt-5.6-luna' },
  // No verified Gemini CLI is available in the baseline environment. Null means
  // "use the CLI default"; existing user IDs and legacy aliases still merge in.
  gemini: { default: null, fast: null, cheap: null },
};
 ...
 * @param {string} agent
 * @returns {{ default: string|null, fast: string|null, cheap: string|null }|null}
 */
...
export function getReasoningCapsMap() {
  ...
  // Current Codex catalogs expose effort controls for the listed GPT models.
  map['gpt-'] = { type: 'effort', levels: ['low', 'medium', 'high', 'xhigh', 'max', 'ultra'], default: 'medium' };

  // OpenAI o-series kept for configurations that still use them.
  ...
  map['opus'] = { type: 'effort', levels: ['low', 'medium', 'high', 'xhigh', 'max'], default: 'high' };
  map['sonnet'] = { type: 'effort', levels: ['low', 'medium', 'high', 'xhigh', 'max'], default: 'high' };
  map['fable'] = { type: 'effort', levels: ['low', 'medium', 'high', 'xhigh', 'max'], default: 'high' };
  map['claude-'] = { type: 'effort', levels: ['low', 'medium', 'high', 'xhigh', 'max'], default: 'high' };
...
export function getConciergeFallbackChain() {
  ...
  const chain = [];
  const primaryProvider = providerForModel(concierge.model);
  if (primaryProvider) {
    chain.push({ provider: primaryProvider, model: concierge.model });
  }
  ...
  for (const modelId of concierge.models) {
    ...
    if (provider) chain.push({ provider, model: modelId });
  }
