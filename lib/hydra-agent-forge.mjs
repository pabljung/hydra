...
- security-reviewer (claude): Security focus, OWASP methodology, severity ratings, remediation guidance. Scores: security=0.98, review=0.92, analysis=0.85.
- test-writer (codex): Test strategy, edge cases, coverage. Scores: testing=0.98, implementation=0.65.
- researcher (claude): Systematic exploration, hypothesis-driven, evidence-based. Scores: research=0.98, analysis=0.90.
...
Virtual: security-reviewer (claude), test-writer (codex), doc-generator (claude), researcher (claude), evolve-researcher (claude)
...
export async function runForgePipeline(description, codebaseCtx = null, opts = {}) {
  ...
  const analystAgent = getRoleConfig('analyst').agent || 'claude';
  const architectAgent = getRoleConfig('architect').agent || 'claude';

  // Phase 1: ANALYZE
  onPhase('analyze', 'running');
  const analyzePrompt = buildAnalyzePrompt(description, profile);
  const analyzeResult = await executeAgent(analystAgent, analyzePrompt, {
    timeoutMs,
    useStdin: true,
    maxOutputBytes: 64 * 1024,
    hubCwd: process.cwd(),
    hubProject: path.basename(process.cwd()),
    hubAgent: `${analystAgent}-forge`,
  });
  ...
  const designResult = await executeAgent(architectAgent, designPrompt, {
    timeoutMs,
    useStdin: true,
    maxOutputBytes: 64 * 1024,
    hubCwd: process.cwd(),
    hubProject: path.basename(process.cwd()),
    hubAgent: `${architectAgent}-forge`,
  });
  ...
  const critiqueResult = await executeAgent(analystAgent, critiquePrompt, {
    timeoutMs,
    useStdin: true,
    maxOutputBytes: 64 * 1024,
    hubCwd: process.cwd(),
  ...
  const refineResult = await executeAgent(architectAgent, refinePrompt, {
    timeoutMs,
    useStdin: true,
    maxOutputBytes: 64 * 1024,
    hubCwd: process.cwd(),
