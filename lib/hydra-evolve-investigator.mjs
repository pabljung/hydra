 ...
 *
 * When an evolve phase fails, this module calls the configured Hydra role through
 * the installed agent CLI, with normal model and agent recovery.
 *
 * Diagnosis buckets:
...
import { isAgentAvailable, resolveAvailableAgent } from './hydra-capabilities.mjs';
...
function getInvestigatorConfig() {
  ...
  const role = getRoleConfig('investigator');
  config = {
    enabled: inv.enabled !== false,
    agent: inv.agent || role.agent || 'codex',
    model: inv.model ?? role.model ?? null,
    reasoningEffort: inv.reasoningEffort || role.reasoningEffort || 'high',
...
export function initInvestigator(overrides = {}) {
  ...
  if (overrides.agent) cfg.agent = overrides.agent;
...
/**
 * Check if the investigator role has an installed agent or configured fallback.
 */
export function isInvestigatorAvailable() {
  const cfg = getInvestigatorConfig();
  if (!cfg.enabled) return false;
  const hydraCfg = loadHydraConfig();
  return isAgentAvailable(cfg.agent, hydraCfg) || Boolean(resolveAvailableAgent(cfg.agent, hydraCfg));
}
...
Diagnose this failure and provide a structured recommendation.`;
  ...
  const investigatorPrompt = `${buildSystemPrompt()}\n\n${userMessage}`;

  try {
    const result = await executeAgentWithRecovery(cfg.agent, investigatorPrompt, {
      cwd: process.cwd(),
      modelOverride: cfg.model || undefined,
      reasoningEffort: cfg.reasoningEffort,
      permissionMode: 'read-only',
      phaseLabel: 'investigator',
      useStdin: true,
    });
    if (!result.ok) throw new Error(result.error || result.stderr || 'Investigator agent failed');

    const fullResponse = result.output || result.stdout || '';
